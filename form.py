...
def get_available_slots(driver, target_date):
    ...
    date_header = driver.find_element(By.XPATH, f"//*[contains(text(), '{target_date}')]")
...
def auto_click_plan(driver, plan, target_date, webhook_url):
    try:
        for start, end, venue_name in plan:
            ...
            # 找所有包含時段的按鈕
            candidate_btns = driver.find_elements(By.XPATH, f"//button[contains(., '{target_time}') and contains(., '申請')]")
            ...
            if not found:
                raise Exception(f"找不到 {target_date} 的 {target_time} 申請按鈕，可能已被搶走！")
